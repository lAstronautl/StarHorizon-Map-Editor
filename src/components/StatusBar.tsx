...
export const StatusBar: React.FC<Props> = ({ state, cursorTileX, cursorTileY, statusMessage }) => {
  ...
  return (
    <div className="flex items-center h-6 bg-surface border-t border-subtle px-3 text-[10px] text-muted gap-4">
      <span>Тайл: (<span className="text-primary">{cursorTileX}</span>, <span className="text-primary">{cursorTileY}</span>)</span>
      <span>{tileId}</span>
      {grid.width > 0 && <span>Сетка: {grid.width} x {grid.height}</span>}
      <span>Сущности: {entities.length}</span>
